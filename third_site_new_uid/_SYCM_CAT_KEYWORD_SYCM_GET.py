...
def cat_url(cid,devid,token):
        url = 'http://sycm.taobao.com/excel/IndustryKeywordExcel.do?device='+str(devid)+'&cateId='+str(cid)+'&pageSize=100&orderBy=hotIndex&desc=&date=1&page=1&sycmToken='+str(token)+'&isHot=true'
        return url


def get_sycm_cat_keyword(uid,token,host,port,user,password,database,initial=0):
...
##        print((project[0]))
        for devid in (1,2):        
                openRecruit(cat_url(project[0],devid,token))
                time.sleep(3) 
