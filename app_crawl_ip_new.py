...
def GenerateURL(pageno):
    ...
    url = 'http://www.xici.net.co/nt/'+str(pageno)
    #url = 'http://www.xici.net.co/nn/'+str(pageno)
...
def getData(pageno):
    ...
    for tr in range(0,len(rows)):
        ...
        try:
            ip = re.findall('>(\d+.\d+.\d+.\d+)<',str(cols[2]))[0]
        except:
            ip = 'notthis'
        try:
            port = re.findall('>(\d+)<',str(cols[3]))[0]
        except:
            port = '-1'
        try:
            try:
                fen = int(re.findall('(\d+)分',str(cols[7]))[0])*60000
            except:
                fen = 0
            try:
                miao = int(re.findall('(\d+)秒',str(cols[7]))[0])*1000
            except:
                miao = 0
            try:
                haomiao = int(re.findall('(\d+)毫秒',str(cols[7]))[0])
            except:
                haomiao = 0
