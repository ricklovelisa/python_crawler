...
def getSubTask(host,user,password,database,r_table,query_rule):
    # keyword,pagenumber = 1,ratesum = "",location = "",cat = "",brand = "",ppath = "",sort = "sale-desc")
    db=MySQLdb.connect(host=host,user=user,passwd=password,db=database,charset="utf8")
    dbconn=db.cursor()
    try:
        queryTask = "SELECT uid_encrypted,attribute FROM %s.%s %s" % (database,r_table,query_rule)
        dbconn.execute(queryTask)
        result = dbconn.fetchall()
        db.commit()
    except Exception,e:
        try:
            queryTask = "SELECT uid_encrypted,'' FROM %s.%s %s" % (database,r_table,query_rule)
            dbconn.execute(queryTask)
            result = dbconn.fetchall()
            db.commit()
        except Exception,e:
...
def openPage(uid_encrypted,rule,pageno,ip):
    ...
    # rater = 3 给予他人的评价
    rater = 3
    url = 'http://rate.taobao.com/member_rate.htm?a=1&_ksTS=1406562448151_176&callback=jsonp_reviews_list&content=&result=&from=rate&user_id='+str(uid_encrypted)+'&identity=1&rater='+str(rater)+'&page='+str(pageno)
    url_referer= 'http://rate.taobao.com/user-rate-'+str(uid_encrypted)+'.htm?spm=a220o.1000855.d4918101.2.Vd9PIB'
    print("###########connect: ",url)
    # 加载 URL 头
    headers = {
        "GET":url,
...
def getSingleData(uid_encrypted,rule,pageno):
    ...
    updatetime = getDate()
    while taskLife >0:
        ip = getIp('localhost','root','root','etc_crawl','ip_proxy')
        try:
        ...
        if pageJsonData[1] == 1:
            ...
            if ipOk == 0 or sleep > 3:
                print("Banned 1")
                markBannedIp('localhost','root','root','etc_crawl','ip_proxy',ip)
            else:
                if sleep == 1:
...
def main():
    conf_file = sys.path[0]+"/crawl.conf"
